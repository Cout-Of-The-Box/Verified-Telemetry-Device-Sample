...
uint32_t vt_adc_channel_sensor_1 = ADC_CHANNEL_8;
uint32_t vt_adc_channel_sensor_2 = ADC_CHANNEL_5;
...
GPIO_TypeDef* vt_gpio_port_sensor_2 = GPIOC;

uint16_t vt_gpio_pin_sensor_1 = GPIO_PIN_2;
uint16_t vt_gpio_pin_sensor_2 = GPIO_PIN_13;
    ...
    uint16_t adc_id, void* adc_controller, void* adc_channel, uint16_t* adc_resolution, float* adc_ref_volt)
{
    if (adc_id == SAMPLE_INTERNAL_ADC_TYPE_ID)
    {
        ...
        vt_adc_controller->Init.ClockPrescaler        = ADC_CLOCK_SYNC_PCLK_DIV2;
        vt_adc_controller->Init.Resolution            = ADC_RESOLUTION_12B;
        vt_adc_controller->Init.ScanConvMode          = DISABLE;
...
uint16_t vt_tick_init(uint16_t* max_value, uint16_t* resolution_usec)
{
    ...
    vt_tick_timer.Instance               = TIM13;
    vt_tick_timer.Init.Prescaler         = (96 * default_tick_resolution) - 1;
